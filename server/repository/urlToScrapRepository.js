...
export default class UrlToScrapRepository {
  ...
  async deleteUrlToScrapRepository(id) {
    await collections.userWebsitesRelationModel
      ...
      .then((r) => r)
      .catch((e) => {
        throw new OccurredUnknownErrorException(
          JSON.stringify({ data: "Occured unknown error" })
        );
      });
  }
  ...
  async addUrlToScrapRepository(url, storeId) {
    ...
    let collectionResponse =
      await websiteService.getCollectionByWebsiteNameFromWeb(url);
    if (collectionResponse.length > 0) {
      await websiteService.upsertWebSitesAllCollections(
        url,
        collectionResponse
      );
    }
    ...
    let faviconResponse =
      await websiteService.getFaviconUrlByWebsiteNameFromWeb(url);
    ...
    await collections.userWebsitesRelationModel
      ...
      .then((resp) => {
        ...
        if (response.length > 0) {
          throw new IsNotValidUrlException(
            JSON.stringify({ data: "This url has already exists" })
          );
        }
      })
      .catch((e) => {
